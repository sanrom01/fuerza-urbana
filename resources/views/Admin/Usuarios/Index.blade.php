                ...
                <tr @if($u->trashed()) style="opacity:.5;background:#fdf5f5" @endif>
                    ...
                    <td class="text-end pe-4">
                        ...
                        <form action="{{ route('admin.usuarios.restore', $u->id) }}" method="POST" class="d-inline">
                            @csrf <button class="btn btn-sm btn-outline-success"><i class="bi bi-arrow-counterclockwise"></i></button>
                        </form>
                        @elseif($u->id !== auth()->id())
...
<div class="d-md-none">
    @forelse($usuarios as $u)
    <div class="mobile-card" @if($u->trashed()) style="opacity:.6" @endif>
        <div class="mobile-card-header">
            <div>
                <div class="fw-700">{{ $u->name }}</div>
                <div class="text-muted" style="font-size:.8rem">{{ $u->email }}</div>
                <div class="text-muted" style="font-size:.75rem">Desde {{ $u->created_at->format('d/m/Y') }}</div>
            </div>
            <div class="text-end">
                <span class="badge {{ $u->role=='admin' ? 'bg-danger' : 'bg-light text-dark border' }} d-block mb-1">
                    {{ $u->role }}
                </span>
                @if($u->trashed())
        ...
        <div class="d-flex gap-2 mt-2">
            <a href="{{ route('admin.usuarios.show', $u) }}" class="btn btn-sm btn-outline-secondary flex-grow-1">
                <i class="bi bi-eye me-1"></i>Ver
            </a>
            <a href="{{ route('admin.usuarios.edit', $u) }}" class="btn btn-sm btn-outline-primary flex-grow-1">
                <i class="bi bi-pencil me-1"></i>Editar
            </a>
        </div>
    </div>
    @empty
    <div class="text-center text-muted py-4">No se encontraron usuarios.</div>
    @endforelse
